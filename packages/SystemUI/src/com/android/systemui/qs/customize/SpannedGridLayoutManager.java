 ...
 *
 * droid-ng: I fixed some bugs and made some more bugs. There used to be a more useful summary here, but...
 *           I gave up trying to summarize my changes. Just search for "droid-ng" and you'll find everything
 *           Original file: https://github.com/nickbutcher/plaid/blob/grid_span/app/src/main/java/io/plaidapp/ui/recyclerview/SpannedGridLayoutManager.java
 *           P.S.: this is messy and buggy to unexplainable extends, don't show this to QA
 */
...
// droid-ng remove Keep
import androidx.annotation.NonNull; // droid-ng androidx
import androidx.recyclerview.widget.ItemTouchHelper; // droid-ng add
import androidx.recyclerview.widget.LinearSmoothScroller; // droid-ng androidx
import androidx.recyclerview.widget.RecyclerView; // droid-ng androidx
 ...
 */
public class SpannedGridLayoutManager extends RecyclerView.LayoutManager implements ItemTouchHelper.ViewDropHandler { //droid-ng add implements ItemTouchHelper.ViewDropHandler
	...
	// droid-ng removed XML constructor
	...
	public int getSpanCount() { // droid-ng add
		return columns; // droid-ng add
	} // droid-ng add

	public void setSpanCount(int columns) { // droid-ng add
		this.columns = columns; // droid-ng add
		removeAllViews(); // droid-ng add
		requestLayout(); // droid-ng add
	} // droid-ng add
	...
	public static class LayoutParams extends RecyclerView.LayoutParams {
		...
		int column; // droid-ng add
		int row; // droid-ng add
		int columnSpan;
		int rowSpan;
	...
	public int scrollVerticallyBy(int dy, RecyclerView.Recycler recycler, RecyclerView.State state) {
		...
		int scrolled;
		View vv = getChildAt(0); //droid-ng move to vv variable
		if (vv == null) return 0; //droid-ng add
		int top = getDecoratedTop(vv); //droid-ng move to vv variable
		...
		if (dy < 0) { // scrolling content down
			...
			if (top - scrolled >= 0) { // new top row came on screen
				...
				if (newRow >= 0) {
					if (getFirstPositionInSpannedRow(firstVisibleRow) == getFirstPositionInSpannedRow(newRow)) { //droid-ng add
						firstVisibleRow--; //droid-ng add
					} else { //droid-ng add
			...
			View v = getChildAt(firstPositionOfLastRow - firstVisiblePosition);//droid-ng move to v variable
			if (v != null) {//droid-ng add
				int lastRowTop = getDecoratedTop(v);//droid-ng move to v variable
				if (lastRowTop - scrolled > getHeight()) { // last spanned row scrolled out
					if (getFirstPositionInSpannedRow(lastVisibleRow) == getFirstPositionInSpannedRow(lastVisibleRow - 1)) { //droid-ng add
						lastVisibleRow--; //droid-ng add
					} else { //droid-ng add
			...
			if (lastVisiblePosition == getItemCount() - 1) { // is at end of content
				int scrollRange = bottom - getHeight() + getPaddingBottom(); //droid-ng remove Math.max to prevent overfling
			...
			if ((bottom - scrolled) < getHeight() && lastVisibleRow != 0) { // new row scrolled in
				...
				if (nextRow < getSpannedRowCount()) {
					if (getFirstPositionInSpannedRow(lastVisibleRow) == getFirstPositionInSpannedRow(nextRow)) { //droid-ng add
						lastVisibleRow = nextRow; //droid-ng add
					} else { //droid-ng add
						int startOffset = top - (firstVisibleRow * cellHeight);
			...
			if (bottomOfFirstRow - scrolled < 0) { // first spanned row scrolled out
				if (getFirstPositionInSpannedRow(firstVisibleRow) == getFirstPositionInSpannedRow(firstVisibleRow + 1)) { //droid-ng add
					firstVisibleRow++; //droid-ng add
				} else { //droid-ng add
	...
	public void scrollToPosition(int position) {
		if (cells == null) return; // droid-ng add
		if (position >= getItemCount()) position = getItemCount() - 1;

		scrollOffsetMy = 0; //droid-ng add
	...
	@Override //droid-ng add
    public void prepareForDrop(@NonNull View view, @NonNull View target, int x, int y) { //droid-ng add
        assertNotInLayoutOrScroll("Cannot drop a view during a scroll or layout calculation"); //droid-ng add
        final int toPos = getPosition(target); //droid-ng add
		final int minTop = getDecoratedTop(target); //droid-ng add
		if (minTop <= getPaddingTop()) { //droid-ng add
			scrollOffsetMy = minTop - getPaddingTop(); //droid-ng add
			scrollOffsetMy += getDecoratedMeasuredHeight(target); //droid-ng add
		} //droid-ng add
		final int maxBottom = getDecoratedBottom(target); //droid-ng add
		if (maxBottom >= getHeight() - getPaddingBottom()) { //droid-ng add
			scrollOffsetMy = maxBottom - (getHeight() - getPaddingBottom()); //droid-ng add
			scrollOffsetMy -= getDecoratedMeasuredHeight(target); //droid-ng add
		} //droid-ng add
		if (scrollOffsetMy != 0) { //droid-ng add
			//resetVisibleItemTracking(); //droid-ng add
			//forceClearOffsets = true; //droid-ng add
			//removeAllViews(); //droid-ng add
			requestLayout(); //droid-ng add
		} //droid-ng add
    } //droid-ng add

	private int scrollOffsetMy = 0; //droid-ng add

	@Override
	public int computeVerticalScrollOffset(RecyclerView.State state) {
		if (getChildCount() == 0) return 0;
		return getPaddingTop() + (firstVisibleRow * cellHeight) - getDecoratedTop(getChildAt(0)) + scrollOffsetMy; //droid-ng add scrollOffsetMy
	}
	...
	private void calculateCellPositions(RecyclerView.Recycler recycler, RecyclerView.State state) {
		...
		for (int position = 0; position < itemCount; position++) {
			...
			boolean fits = false; //droid-ng add
			while (!fits) { //droid-ng add
				fits = true; //droid-ng add
				for (int i = column; i < column + spanInfo.columnSpan; i++) { //droid-ng add
					if (rowHWM[i] > row) { //droid-ng column->i, while->if
						column++;
						if (column + spanInfo.columnSpan > columns) {
							row++;
							recordSpannedRowStartPosition(row, position);
						...
						fits = false; //droid-ng add
					}
					if (!fits) break; //droid-ng add
				} //droid-ng add
			} //droid-ng add
	...
	private int getFirstPositionInSpannedRow(int rowIndex) {
		return firstChildPositionForRow.get(Math.min(firstChildPositionForRow.size() - 1, rowIndex)); //droid-ng fix OOB
	}
	...
	private int layoutRow(
		...
		for (int position = firstPositionInRow;
			...
			lp.column = cell.column; // droid-ng add
			lp.row = cell.row; // droid-ng add
